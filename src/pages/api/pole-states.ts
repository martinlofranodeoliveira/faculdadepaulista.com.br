...
export const GET: APIRoute = async () => {
  try {
    const items = (await fetchPublicPolesApi('pole-states', 'Não foi possível carregar os estados.'))
    ...
    return new Response(
      JSON.stringify({
        message: error instanceof Error ? error.message : 'Não foi possível carregar os estados.',
      }),
      {
        status: 500,
        headers: {
